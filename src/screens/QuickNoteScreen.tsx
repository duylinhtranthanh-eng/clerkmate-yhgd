...
import { Badge, Card, Checkbox, Notice } from '../components/Ui'
...
export function QuickNoteScreen({
  ...
  const showSuggestions = (
    ...
    setSelected(
      new Set(found.filter((s) => !s.alreadyPresent && s.confidence !== 'low').map((s) => s.id)),
    )
  }
  ...
  const saveDraftNote = (): string | null => {
    ...
    next.quickNotes.push({ id, createdAt: new Date().toISOString(), text, filedInto: [], archived: false })
    replace(next)
    setDraft('')
    return id
  }
  ...
  const applySelected = () => {
    ...
    toast(`Đã đưa ${chosen.length} mục vào bệnh án.`)
  }
  ...
  return (
    <div className="content">
      <Card
        title="Ghi chú nhanh"
        hint="Viết tự nhiên như trên giấy nháp. Không cần đúng thứ tự, không cần đầy đủ."
      >
        <textarea
          className="notepad"
          value={draft}
      ...
      {notes.length > 0 && (
        <Card title={`Ghi chú đã lưu (${notes.length})`} className="card--flat">
          <div className="stack">
            {notes.map((n) => (
